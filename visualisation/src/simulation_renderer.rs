...
pub fn render_simulation(
        ...
        .filter(|snap| enable_debug_rendering || !snap.validity_time_range.is_later(time))
    {
        ...
        let cliped_ball_shapes = snap.portal_traversals.iter()
            .filter(|traversal| enable_debug_rendering || !traversal.time_range.is_later(time))
        ...
        if enable_debug_rendering {
            ...
            if !snap.portal_traversals.is_empty() {
                let is_real = snap.portal_traversals.iter().any(|traversal| traversal.time_range.contains(&time));
                let is_later = snap.portal_traversals.iter().any(|traversal| traversal.time_range.is_later(time));
